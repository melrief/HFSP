...
public class HFSPFakeJobInProgress extends JobInProgress {
  ...
  public HFSPFakeJobInProgress(JobConf jobConf,
      FakeClock clock, HFSPFakeTaskTrackerManager taskTrackerManager, 
...
//    this.jobHistory = new FakeJobHistory();
  ...
  public synchronized void initTasks() throws IOException {
    ...
    for (int i = 0; i < numMapTasks; i++) {
      ...
      maps[i] = new HFSPFakeTaskInProgress(this.getJobID(), 
          this.jobTracker,
          true, i,
          this.getJobConf(), this, inputLocations, split, this.clock);
    ...
    for (int i = 0; i < numReduceTasks; i++) {
      reduces[i] = new HFSPFakeTaskInProgress(getJobID(), 
          this.jobTracker, false, i,
          getJobConf(), this, null, split, clock);
    }
  ...
  public Task obtainNewMapTask(final TaskTrackerStatus tts, int clusterSize,
    ...
    return this.obtainNewMapTask(tts, clusterSize, numUniqueHosts, LocalityLevel.ANY.ordinal());
  }
  ...
  public synchronized Task obtainNewNodeLocalMapTask(TaskTrackerStatus tts,
                                                     ...
                                                     int numUniqueHosts)
  throws IOException {
    return this.obtainNewMapTask(tts, clusterSize, numUniqueHosts, LocalityLevel.NODE.ordinal());
  }
  ...
  throws IOException {
    return this.obtainNewMapTask(tts, clusterSize, numUniqueHosts, LocalityLevel.RACK.ordinal());
  }
  ...
  public Task obtainNewReduceTask(final TaskTrackerStatus tts,
    ...
    for (int reduce = 0; reduce < reduces.length; reduce++) {
      HFSPFakeTaskInProgress tip = 
        (HFSPFakeTaskInProgress) reduces[reduce];
      if (!tip.isRunning() && !tip.isComplete()) {
        TaskAttemptID attemptId = getTaskAttemptID(tip);
